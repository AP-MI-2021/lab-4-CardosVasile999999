...
def get_all_elements_with_integer_part_being_a_divisor_of_fractional_part(lst):
    """
    Functie ce are ca parametrii lista de float-uri si returneaza elementele care au partea intreaga divizor
    al partii fractionale
...
def test_get_all_elements_with_integer_part_being_a_divisor_of_fractional_part():
    ...
    assert get_all_elements_with_integer_part_being_a_divisor_of_fractional_part([6]) == []
    assert get_all_elements_with_integer_part_being_a_divisor_of_fractional_part([2.7, 8, 2, 4]) == []
    ...
    for element in lst:
        ...
        for j in elementu:
            if j == '-':
                ...
                aux = aux + 'noua'
            elif j == '.' and int(element) != element:
                aux = aux + 'virgula'
        result.append(aux)
    return result


def test_get_characther_into_word():
    assert get_characther_into_word([]) == []
    assert get_characther_into_word([-123, 1.3, 8]) == ['minusunudoitrei', 'unuvirgulatrei', 'opt']
    assert get_characther_into_word([-5.69]) == ['minuscincivirgulasasenoua']
